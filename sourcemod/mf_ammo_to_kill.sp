...
new Handle:h_aak_Enabled, Handle:h_aak_TeamKill, Handle:h_aak_Add_0Slot, Handle:h_aak_Add_1Slot , Handle:h_aak_AmmoRate, Handle:h_aak_AddGranade, Handle:h_aak_Access, Handle:h_aak_MsgEnabled;
new bool:b_akk_enabled, bool:b_akk_teamlill, bool:b_akk_0slot,  bool:b_akk_1slot, bool:b_akk_3slot, bool:b_akk_msg;
new Float:i_akk_rate;
...
{
	b_akk_enabled 	= GetConVarBool(h_aak_Enabled);
	b_akk_teamlill 	= GetConVarBool(h_aak_TeamKill);
	b_akk_msg 		= GetConVarBool(h_aak_MsgEnabled);
	b_akk_0slot 	= GetConVarBool(h_aak_Add_0Slot);
	b_akk_1slot		= GetConVarBool(h_aak_Add_1Slot);
	b_akk_3slot 	= GetConVarBool(h_aak_AddGranade);
	i_akk_rate		= GetConVarFloat(h_aak_AmmoRate);
	return 0;
}
...
{
	if (convar 	== h_aak_Enabled) 			b_akk_enabled 	= GetConVarBool(convar);
	else if (convar == h_aak_TeamKill)		b_akk_teamlill 	= GetConVarBool(convar);
	else if (convar == h_aak_MsgEnabled) 	b_akk_msg 	= GetConVarBool(convar);
	else if (convar == h_aak_Add_0Slot) 	b_akk_0slot 	= GetConVarBool(convar);
	else if (convar == h_aak_Add_1Slot) 	b_akk_1slot		= GetConVarBool(convar);
	else if (convar == h_aak_AddGranade) 	b_akk_3slot 	= GetConVarBool(convar);
	else if (convar == h_aak_AmmoRate) 		i_akk_rate 		= GetConVarFloat(convar);
...
public OnPlayerDeath(Handle:event, String:name[], bool:dontBroadcast)
{
	...
	if (GetClientTeam(victim) == GetClientTeam(attacker) && !b_akk_teamlill)  return 0; //проверяем на тимкилл
	...
	//PrintToChatAll("Player %N kill %N witch %s", attacker, victim, AttackerWeapon);
	if(b_akk_0slot && IsValidEdict(weapon_0slot))
		{
			for(new i = 0; i<18;i++)
			{
				...
				new add_ammo =  RoundFloat((float(WeaponClips[i]) - float(rem_ammo)) * i_akk_rate);
				SetWeaponClip(weapon_0slot, rem_ammo + add_ammo);
				if(b_akk_msg) PrintHintText(attacker, "Боезапас пополнен (%i)", add_ammo);
			}
		}
	if(b_akk_1slot && IsValidEdict(weapon_1slot))
		{
			for(new i = 18; i<24;i++)
			{
				if (!StrEqual(WeaponNames[i], AttackerWeapon, false)) 
				...
				if(b_akk_msg) PrintHintText(attacker, "Боезапас пополнен (%i патрон(а)/(ов))", add_ammo);
			}
		}
	if(StrEqual(weapon_dead, "hegrenade", false) && b_akk_3slot)
	{
		GivePlayerItem(attacker, "weapon_hegrenade"); 
		if(b_akk_msg) PrintHintText(attacker, "Вы получили боевую гранату!");
	}
	return 0;
}

SetWeaponClip(weapon, value)
{
